...
#define NUMLINES 30
#define MAXLINE 1000
#define SONG_FILENAME_LENGTH 200
...
void gameover(int rounds, char song[], int song_length) {
    int highscore;
    char *highscore_path;
    ...
    if (doesFileExist(highscore_path)) {
        ...
        if (rounds > highscore) {
            ...
            fclose(writefile);
            printf("Ny high score!! Du klarte %d runder! Gratulerer!!\n", rounds);
        } else {
    ...
    printf("Du brukte i gjennomsnitt %f sekunder på å finne et semikolon!", song_length / (float)rounds);
    free(highscore_path);
}

int get_song(int song_idx, int *song_length, char song_filename[]) {
    FILE *file = fopen("songs.txt", "r");
    char line[SONG_FILENAME_LENGTH];
    char song[SONG_FILENAME_LENGTH];
    int in_song;
    int in_song_length;
    int i;
...
int main(int argc, char const *argv[])
{
    ...
    char song_filename[SONG_FILENAME_LENGTH];

    char *song_path;
    char *play_song_command;
    int song_duration;
    ...
    file_size = ftell(file);  // because a char is one byte, this is also the number of chars in the file
    rewind(file);
    ...
    song_path = malloc(strlen(song_filename) + strlen("songs/"));
    if (song_path == NULL) {
    ...
    play_song_command = malloc(strlen("cvlc --play-and-exit \"") + strlen(song_path) + strlen(" &> /dev/null &"));
    strcpy(play_song_command, "cvlc --play-and-exit \"");
    strcat(play_song_command, song_path);
    strcat(play_song_command, "\" &> /dev/null &");
    system(play_song_command);
    sleep(1);  // wait for for the output of vlc
    system("clear");
    ...
    while (1) {
        ...
        while (1) {
            ...
            knr_getline(input);
            if (input[0] == 'q') {
                system("pkill vlc");  // Stop the song
                gameover(current_round, song_filename, song_duration);
                return 0;
            }
